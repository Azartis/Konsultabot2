...
export const VoiceHelper = {
  ...
  on(event, callback) {
    ...
    try {
      // @react-native-voice/voice uses addListener method (EventEmitter pattern)
      // Try addListener first (preferred method)
      if (typeof Voice.addListener === 'function') {
        Voice.addListener(event, callback);
        console.log(`✅ Added listener for ${event} using addListener()`);
        return;
      }
      
      // Fallback: Map event names to Voice event handler properties
      const eventMap = {
      ...
      // Set the event handler directly on Voice object (legacy method)
      if (Voice[voiceEvent] !== undefined || typeof Voice[voiceEvent] === 'function') {
  ...
  removeAllListeners() {
    ...
    try {
      ...
      // Try removeAllListeners method first (EventEmitter pattern)
      if (typeof Voice.removeAllListeners === 'function') {
        Voice.removeAllListeners();
        console.log('✅ Removed all listeners using removeAllListeners()');
        return;
      }
      
      // Fallback: Manually remove listeners - but check Voice is not null before each assignment
      try {
        ...
        console.log('✅ Removed all listeners manually');
      } catch (assignError) {
