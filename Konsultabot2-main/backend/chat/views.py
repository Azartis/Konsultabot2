...
def get_gemini_response(message: str, language: str = "english"):
    ...
    print(f"[TEST] Testing chat with Gemini: '{message}'")
    print(f"[CHECK] Checking internet: {has_internet()}")
    ...
    try:
        api_key = os.getenv("GOOGLE_API_KEY", "").strip()
        print(f"[KEY] API Key present: {bool(api_key)} (length: {len(api_key) if api_key else 0})")
        ...
        print(f"[SEND] Sending to Gemini: {prompt[:50]}...")
        response = model.generate_content(prompt)
        
        if hasattr(response, 'text') and response.text:
            response_text = response.text.strip()
            print(f"[RESPONSE] Gemini response length: {len(response_text)}")
            print(f"[PREVIEW] Response preview: {response_text[:100]}...")
            return {"response": response_text, "mode": "online"}
        else:
            print(f"[ERROR] No text in response: {response}")
            return None
...
print("[OK] Direct Gemini integration loaded")
...
**Printer troubles:** Won't turn on, paper jams, printing quality issues, showing offline
...
The more details you can share, the better I can help you get this sorted out! Don't worry if you're not sure about technical terms - just describe what you're experiencing in your own words."""
        ...
        if GEMINI_AVAILABLE and (result['confidence'] < 0.9 or result['mode'] in ['basic_response', 'fallback', 'greeting'] or is_general_question):
            print(f"[GEMINI] Trying Gemini for: '{message}' (confidence: {result['confidence']}, mode: {result['mode']}, general: {is_general_question})")
            try:
...
def test_chat_gemini(request):
    ...
    debug_info = []
    debug_info.append(f"[TEST] Testing chat with Gemini: '{message}'")
