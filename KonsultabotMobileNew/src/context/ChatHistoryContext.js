  ...
  const saveChats = async () => {
    try {
      ...
      if (chatsToSave.length > 0) {
        // Sanitize messages before saving to avoid serialization errors
        const sanitizedChats = chatsToSave.map(chat => ({
          ...chat,
          messages: chat.messages.map(msg => {
            // Create a clean message object with only serializable properties
            const cleanMsg = {
              id: msg.id,
              text: msg.text || '',
              sender: msg.sender || 'bot',
              timestamp: msg.timestamp ? (msg.timestamp instanceof Date ? msg.timestamp.toISOString() : msg.timestamp) : new Date().toISOString(),
            ...
            // Remove any undefined or null values to keep it clean
        ...
        await AsyncStorage.setItem(storageKey, JSON.stringify(sanitizedChats));
