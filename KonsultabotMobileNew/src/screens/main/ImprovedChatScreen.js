...
export default function ImprovedChatScreen({ navigation = null }) {
  ...
  // Use useMemo to prevent chatId from changing on every render (which causes infinite loops)
  const chatId = useMemo(() => {
    return currentChatId || `chat_${user?.id || 'guest'}_${Date.now()}`;
  ...
  // Initialize chat on mount - only run once
  useEffect(() => {
    initializeChat();
    initializeSpeechRecognition();
    initializeWakeWordDetection();
    
    // Initialize offline knowledge base (only once)
  ...
  // Load offline messages when chatId or user changes
  useEffect(() => {
    ...
    if (Platform.OS === 'web') {
      ...
      if (WebSpeechRecognition) {
        ...
        recognition.onresult = (event) => {
          ...
          // Mark as voice input - user can review and send manually
      ...
      if (VoiceHelper.isAvailable()) {
        // Silent check - don't log repeatedly
      }
    }
  };
  ...
  useEffect(() => {
    if (currentChatId && messages.length > 0 && updateChatMessages) {
      ...
      if (hasUserMessages) {
        try {
          // Ensure messages are clean before saving
          const cleanMessages = messages.map(msg => ({
            ...
            mode: msg.mode,
            kb_source: msg.kb_source,
            question_count: msg.question_count,
            userQuery: msg.userQuery,
            satisfied: msg.satisfied,
          }));
          updateChatMessages(currentChatId, cleanMessages);
        } catch (error) {
          console.error('Error saving messages:', error);
          // Log specific error if it's related to botMsg
          if (error.message && error.message.includes('botMsg')) {
    ...
    try {
      ...
      if (VoiceHelper.isAvailable()) {
        try {
          ...
          if (!stopError.message?.includes('null') && !stopError.message?.includes('Cannot read property')) {
            console.warn('⚠️ Could not stop VoiceHelper:', stopError.message);
          }
        }
        try {
          VoiceHelper.removeAllListeners();
          ...
          if (!removeError.message?.includes('null') && !removeError.message?.includes('Cannot set property')) {
            console.warn('⚠️ Could not remove VoiceHelper listeners:', removeError.message);
          }
        }
      }
      setIsRecording(false);
    ...
    try {
      ...
      if (ExpoVoiceHelper.isRecording()) {
        ...
        if (audioUri && typeof audioUri === 'string') {
          ...
          try {
            // Send audio to backend for transcription (LemonFox or backend)
            ...
            // Check if it's a network error from the error object
            else if (transcribeError?.code === 'ERR_NETWORK' || errorMessage.includes('Network Error') || errorMessage.includes('timeout')) {
              Alert.alert(
                'Network Error',
            ...
            // Other errors - show the actual error message
              ...
              Alert.alert(
                'Transcription Error',
                errorMessage || 'Could not transcribe audio. Please try typing your message instead.',
                [{ text: 'OK' }]
              );
            }
            setIsTranscribing(false);
      ...
      if (transcript && transcript.trim()) {
        ...
        // Transcription complete - user can review and send manually
      } else {
              ...
              <TouchableOpacity 
                ...
                onPress={() => {
                  ...
                  if (navigation && typeof navigation.navigate === 'function') {
                    ...
                    // Navigation not available - just close the menu
                    console.log('ℹ️ Navigation not available - Settings feature requires navigation');
                  }
                }}
              >
