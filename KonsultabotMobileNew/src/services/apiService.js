...
const getPossibleBackendURLs = () => {
  ...
  if (metroIP) {
    // Try port 80 first (user's backend), then 8000
    urls.push(`http://${metroIP}:80/api`);
  ...
  if (Platform.OS === 'android') {
    ...
    urls.push(
      'http://10.0.2.2:80/api',        // Android emulator port 80
      'http://10.0.2.2:8000/api',      // Android emulator port 8000 (fallback)
    ...
    // Try port 80 first, then 8000
    urls.push(
      'http://localhost:80/api',       // Local development port 80
      'http://localhost:8000/api',     // Local development port 8000 (fallback)
    );
  } else {
    ...
    // Try port 80 first, then 8000
    urls.push(
      `http://${webHost}:80/api`,      // Current host → backend port 80
      `http://${webHost}:8000/api`,    // Current host → backend port 8000
      'http://localhost:80/api',       // Explicit localhost port 80
      'http://localhost:8000/api',     // Explicit localhost port 8000
      'http://127.0.0.1:80/api',       // Loopback port 80
      'http://127.0.0.1:8000/api',     // Loopback port 8000
    );
  }
...
const discoverBackendURL = async () => {
  ...
  try {
    ...
    if (ngrokUrl && typeof ngrokUrl === 'string' && (ngrokUrl.includes('ngrok.io') || ngrokUrl.includes('ngrok-free.dev') || ngrokUrl.includes('ngrok.app'))) {
      // Use ngrok URL as-is without adding /api
      const cleanNgrokUrl = ngrokUrl.replace(/\/api\/?$/, '').replace(/\/$/, '');
      try {
        // Test the health endpoint (try both with and without /api for compatibility)
        const healthEndpoints = [`${cleanNgrokUrl}/api/health/`, `${cleanNgrokUrl}/health/`];
        let healthUrl = null;
        ...
        if (healthUrl) {
          console.log('🌐 Using Ngrok URL for global access:', cleanNgrokUrl);
          cachedBackendURL = cleanNgrokUrl;
          await AsyncStorage.setItem('backend_url', cleanNgrokUrl);
          return cleanNgrokUrl;
        }
      } catch (e) {
        // Even if health check fails, use ngrok URL as it might be a temporary issue
        console.log('⚠️ Ngrok health check failed, but using ngrok URL anyway:', cleanNgrokUrl);
  ...
  let fallbackURL;
  if (Platform.OS === 'web') {
    fallbackURL = 'http://localhost:80/api';  // Localhost for web (port 80)
  ...
  console.log('   1. Make sure backend is running: python manage.py runserver 0.0.0.0:80');
  ...
  console.log('   4. Try accessing http://YOUR_IP:80/api/health/ from phone browser');
  console.log('   5. Check Windows Firewall allows port 80');
...
let initialBaseURL;
if (Platform.OS === 'web') {
  initialBaseURL = 'http://localhost:80/api';  // Web uses localhost port 80
} else {
  ...
  if (ngrokUrl && (ngrokUrl.includes('ngrok') || ngrokUrl.includes('https://'))) {
    initialBaseURL = ngrokUrl.replace(/\/api\/?$/, '').replace(/\/$/, '');
  } else {
    // Fallback to default Ngrok URL
    initialBaseURL = 'https://unmutated-nondeprecatively-bonnie.ngrok-free.dev';
  }
}
...
const getApiUrl = async () => {
  try {
    // Web (browser) - Use localhost port 80
    if (Platform.OS === 'web') {
    ...
    if (ngrokUrl && (ngrokUrl.includes('ngrok') || ngrokUrl.includes('https://'))) {
      // Use ngrok URL as-is without adding /api
      const cleanUrl = ngrokUrl.replace(/\/api\/?$/, '').replace(/\/$/, '');
      console.log(`🌐 Using Ngrok URL from config: ${cleanUrl}`);
      return cleanUrl;
    }
...
const defaultNgrokUrl = Constants.expoConfig?.extra?.apiUrl || 
                         Constants.expoConfig?.extra?.ngrokUrl ||
                         'https://unmutated-nondeprecatively-bonnie.ngrok-free.dev';
let API_BASE_URL = Platform.OS === 'web' ? 'http://localhost:80/api' : defaultNgrokUrl;
...
class ApiService {
  ...
  async login(username, password) {
    try {
      ...
      if (error.code === 'ECONNREFUSED' || 
        ...
        const backendURL = this.api.defaults.baseURL || 'http://192.168.103.243:80/api';
        throw new Error(
          `Cannot connect to backend server at ${backendURL}.\n\n` +
          `Please check:\n` +
          `• Backend is running: python manage.py runserver 0.0.0.0:80\n` +
          `• Device and computer are on the same WiFi network\n` +
          `• Windows Firewall allows port 80\n` +
          `• Try accessing http://192.168.103.243:80/api/health/ in your browser\n\n` +
  ...
  async sendChatMessage(query) {
    try {
      ...
      const response = await this.api.post('/api/chat/simple-gemini/', payload, {
        timeout: 60000 // 60 seconds for AI processing
      });
  ...
  async sendMessage(message, language = 'english', sessionId = null) {
    ...
    try {
      return await this.api.post('/api/chat/send/', payload);
    } catch (error) {
  ...
  async sendV1ChatMessage(query, language = 'english', sessionId = null, additionalData = {}) {
    ...
    try {
      ...
      // For web, always talk to backend on localhost:80 to avoid stale LAN IPs
      if (Platform.OS === 'web') {
        this.api.defaults.baseURL = 'http://localhost:80/api';
      }
      
      console.log('💬 Sending chat message to:', this.api.defaults.baseURL + '/api/v1/chat/');
      const response = await this.api.post('/api/v1/chat/', payload);
  ...
  async getConversationHistory() {
    return this.api.get('/api/v1/chat/history/');
  }

  async getChatSessions() {
    return this.api.get('/api/chat/sessions/');
  }

  async endChatSession(sessionId) {
    return this.api.post('/api/chat/sessions/end/', { session_id: sessionId });
  }

  async getKnowledgeBase(language = 'english', category = null) {
    const params = { language };
    if (category) params.category = category;
    return this.api.get('/api/chat/knowledge/', { params });
  }
  ...
  async getUserKnowledgeBase() {
    try {
      await this.ensureBackendURL();
      const response = await this.api.get('/api/v1/chat/user-kb/');
  ...
  async createUserKnowledgeBaseEntry(entryData) {
    try {
      await this.ensureBackendURL();
      const response = await this.api.post('/api/v1/chat/user-kb/create/', entryData);
      return response.data;
    } catch (error) {
  ...
  async updateUserKnowledgeBaseEntry(entryId, entryData) {
    try {
      await this.ensureBackendURL();
      const response = await this.api.put(`/api/v1/chat/user-kb/${entryId}/`, entryData);
      return response.data;
    } catch (error) {
  ...
  async deleteUserKnowledgeBaseEntry(entryId) {
    try {
      await this.ensureBackendURL();
      const response = await this.api.delete(`/api/v1/chat/user-kb/${entryId}/`);
      return response.data;
    } catch (error) {
  ...
  async syncUserKnowledgeBase(entries) {
    try {
      await this.ensureBackendURL();
      const response = await this.api.post('/api/v1/chat/user-kb/sync/', { entries });
      return response.data;
    } catch (error) {
  ...
  async getCampusInfo(language = 'english', category = null) {
    const params = { language };
    if (category) params.category = category;
    return this.api.get('/api/chat/campus-info/', { params });
  }

  async searchKnowledge(query, language = 'english') {
    return this.api.get('/api/chat/search/', {
  ...
  async testGemini(message = 'What is artificial intelligence?') {
    return this.api.post('/api/chat/test-gemini/', { 
      message, 
      language: 'english' 
    });
  }
  ...
  async checkServer(ip) {
    // Try port 80 first, then 8000 as fallback
    const ports = [80, 8000];
    ...
    throw new Error(`Server not reachable on ports 80 or 8000`);
  }
  ...
  /**
   * Transcribe audio file to text using LemonFox API (primary) or backend (fallback)
   * Uses expo-av for recording and sends to LemonFox API for transcription
   * 
   * @param {string} audioUri - URI/path to the audio file
   * @param {string} language - Language code (default: 'en-US')
   * @returns {Promise<{transcript: string, text: string, confidence: number}>}
   */
  async transcribeAudio(audioUri, language = 'en-US') {
    // Try LemonFox API first (no backend dependencies)
    try {
      return await this.transcribeWithLemonFox(audioUri, language);
    } catch (lemonfoxError) {
      console.warn('⚠️ LemonFox transcription failed, trying backend:', lemonfoxError.message);
      // Fallback to backend transcription
      return await this.transcribeWithBackend(audioUri, language);
    }
  }
   ...
   * @returns {Promise<{transcript: string, text: string, confidence: number}>}
   */
  async transcribeWithLemonFox(audioUri, language = 'en-US') {
    try {
      const { LEMONFOX_API_KEY, LEMONFOX_CONFIG } = require('../config/speechConfig');
      
      if (!LEMONFOX_API_KEY || LEMONFOX_API_KEY === 'YOUR_LEMONFOX_API_KEY_HERE') {
      ...
      if (!audioUri || typeof audioUri !== 'string') {
        throw new Error(`Invalid audioUri: expected string, got ${typeof audioUri}. Value: ${JSON.stringify(audioUri)}`);
      }

      console.log('🍋 Transcribing audio with LemonFox API:', audioUri);
      ...
      // Map language code to LemonFox format
      const languageMap = {
        'en-US': 'english',
        'en': 'english',
        'tl-PH': 'tagalog',
      ...
      const lemonfoxLanguage = languageMap[language] || 'english';
      
      // For React Native, we need to create a file object
      if (Platform.OS !== 'web') {
        // React Native: audioUri is a file path (file:// or content://)
      ...
      // Add language and response format
      formData.append('language', lemonfoxLanguage);
      formData.append('response_format', 'json');
      
      console.log('📤 Sending audio to LemonFox API...');
      
      // Send to LemonFox API
      const response = await axios.post(LEMONFOX_CONFIG.apiUrl, formData, {
        headers: {
          'Authorization': `Bearer ${LEMONFOX_API_KEY}`,
          'Content-Type': 'multipart/form-data',
        },
        timeout: LEMONFOX_CONFIG.timeout,
      });
      
      console.log('✅ LemonFox transcription response:', response.data);
      
      // LemonFox returns { text: "transcribed text" }
      const transcript = response.data?.text || '';
      if (!transcript) {
        throw new Error('No transcript returned from LemonFox API');
      }
      
      return {
        transcript: transcript,
        text: transcript,
        confidence: 0.95, // LemonFox is highly accurate
        language: language,
        method: 'lemonfox',
      };
    } catch (error) {
      ...
      if (error.response?.status === 401) {
        throw new Error('LemonFox API key is invalid. Please check your API key configuration.');
      ...
      throw new Error(error.response?.data?.error || error.message || 'LemonFox transcription failed');
    }
  }

  /**
   * Transcribe audio file to text using backend speech-to-text endpoint (fallback)
   * Uses expo-av for recording and sends to backend for transcription
   * 
   * @param {string} audioUri - URI/path to the audio file
   * @param {string} language - Language code (default: 'en-US')
   * @returns {Promise<{transcript: string, text: string, confidence: number}>}
   */
  async transcribeWithBackend(audioUri, language = 'en-US') {
    try {
      await this.ensureBackendURL();
      ...
      console.log('🌐 Using backend URL:', this.api.defaults.baseURL);
      ...
      // Ensure auth token is available - try to get from AsyncStorage if not set
      if (!this.authToken) {
        try {
          ...
          console.warn('⚠️ Could not retrieve auth token from storage:', storageError);
        }
      }
      
      // Prepare headers - ensure auth token is included
      const headers = {
        'Content-Type': 'multipart/form-data',
      };
      ...
      if (this.authToken) {
        ...
        console.log('🔐 Including auth token in transcription request');
      } else {
        console.warn('⚠️ No auth token available for transcription request - user may need to log in');
      }
      
      // Add ngrok bypass header if needed
      if (this.api.defaults.baseURL && (this.api.defaults.baseURL.includes('ngrok-free.dev') || this.api.defaults.baseURL.includes('ngrok.io') || this.api.defaults.baseURL.includes('ngrok.app'))) {
        headers['ngrok-skip-browser-warning'] = 'true';
      }
      
      // Send to backend transcription endpoint
      const response = await this.api.post('/api/v1/chat/transcribe/', formData, {
        headers: headers,
      ...
      return {
        ...
        language: response.data.language || language,
        method: response.data.method || 'backend',
      };
    } catch (error) {
      ...
      if (error.response?.status === 401) {
        ...
        throw new Error('Authentication required. Please log in again to use voice transcription.');
      } else if (error.response?.status === 400) {
        throw new Error('Invalid audio file format. Please try recording again.');
      } else if (error.response?.status === 413) {
        throw new Error('Audio file too large. Please record a shorter message.');
