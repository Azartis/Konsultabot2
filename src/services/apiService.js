...
const getPossibleBackendURLs = () => {
  ...
  // Base URLs should NOT include /api - it will be added to API paths
  const metroIP = getMetroBundlerIP();
  if (metroIP) {
    urls.push(`http://${metroIP}:8000`);
  ...
  if (Platform.OS === 'android') {
    ...
    urls.push(
      'http://10.0.2.2:8000',       // Android emulator (for testing)
    ...
    urls.push(
      `http://${webHost}:8000`,      // Current host → backend port 8000
      'http://localhost:8000',       // Explicit localhost
      'http://127.0.0.1:8000',       // Loopback alternative
    );
  }
...
const discoverBackendURL = async () => {
  ...
  try {
    ...
    if (ngrokUrl && typeof ngrokUrl === 'string' && (ngrokUrl.includes('ngrok.io') || ngrokUrl.includes('ngrok-free.dev') || ngrokUrl.includes('ngrok.app'))) {
      // Use ngrok URL directly without /api suffix
      try {
        // Test the health endpoint (backend expects /api/health/)
        ...
        if (testResponse.status === 200) {
          console.log('🌐 Using Ngrok URL for global access:', ngrokUrl);
          // Store base URL without /api suffix
          cachedBackendURL = ngrokUrl;
          await AsyncStorage.setItem('backend_url', ngrokUrl);
          return ngrokUrl;
        }
      } catch (e) {
        // Even if health check fails, use ngrok URL as it might be a temporary issue
        console.log('⚠️ Ngrok health check failed, but using ngrok URL anyway:', ngrokUrl);
  ...
  try {
    let stored = await AsyncStorage.getItem('backend_url');
    if (stored && typeof stored === 'string') {
      // Clean up cached URL - remove /api if present
      stored = stored.trim().replace(/\/api\/?$/, '').replace(/\/$/, '');
      ...
      const isLocalIP = stored.includes('192.168.') || stored.includes('10.0.') || stored.includes('127.0.0.1') || stored.includes('localhost');
        ...
        try {
          // stored should not have /api, so just add /api/health/
          const testResponse = await axios.get(`${stored}/api/health/`, {
            timeout: 10000,
  ...
  try {
    let manualURL = await AsyncStorage.getItem('manual_backend_url');
    if (manualURL) {
      // Clean up manual URL - remove /api if present
      manualURL = manualURL.trim().replace(/\/api\/?$/, '').replace(/\/$/, '');
      try {
        const testResponse = await axios.get(`${manualURL}/api/health/`, {
          timeout: 5000,
          headers: { 'Accept': 'application/json' }
        });
        if (testResponse.status === 200) {
  ...
  for (const url of POSSIBLE_BACKEND_URLS) {
    ...
    try {
      ...
      // url should not have /api, so just add /api/health/
  ...
  let fallbackURL;
  if (Platform.OS === 'web') {
    fallbackURL = 'http://localhost:8000';  // Localhost for web
...
let initialBaseURL;
if (Platform.OS === 'web') {
  initialBaseURL = 'http://localhost:8000';  // Web uses localhost
} else {
  ...
  if (ngrokUrl && (ngrokUrl.includes('ngrok') || ngrokUrl.includes('https://'))) {
    // Base URL should NOT include /api - it will be added to API paths
    initialBaseURL = ngrokUrl;
  } else {
    // Fallback to default Ngrok URL (without /api)
...
const getApiUrl = async () => {
  try {
    ...
    let ngrokUrl = Constants.expoConfig?.extra?.ngrokUrl || 
                   Constants.expoConfig?.extra?.apiUrl ||
                   process.env.EXPO_PUBLIC_NGROK_URL;
    
    // Clean up URL - remove /api if present
    if (ngrokUrl && typeof ngrokUrl === 'string') {
      ngrokUrl = ngrokUrl.trim().replace(/\/api\/?$/, '').replace(/\/$/, '');
    }
    
    if (ngrokUrl && (ngrokUrl.includes('ngrok') || ngrokUrl.includes('https://'))) {
      console.log(`🌐 Using Ngrok URL from config: ${ngrokUrl}`);
      return ngrokUrl;
    }
    ...
    // Fallback to default Ngrok URL (from app.config.js) - should NOT include /api
    ...
    // Fallback to Ngrok URL - should NOT include /api
...
// Base URL should NOT include /api - it will be added to individual API paths
const defaultNgrokUrl = Constants.expoConfig?.extra?.apiUrl || 
                         Constants.expoConfig?.extra?.ngrokUrl ||
                         'https://unmutated-nondeprecatively-bonnie.ngrok-free.dev';
let API_BASE_URL = Platform.OS === 'web' ? 'http://localhost:8000' : defaultNgrokUrl;
...
class ApiService {
      ...
      if (error?.code === 'ERR_NETWORK') {
        ...
        try {
          ...
          if (newUrl) {
            ...
            const retryResponse = await this.api.post('/api/v1/chat/', payload);
  ...
  // Upload audio and get transcript (used by ExpoVoiceHelper fallback)
  async transcribeAudio(audioUri, language = 'en-US') {
    try {
      ...
      const response = await this.api.post('/api/v1/transcribe/', formData, {
        headers: {
