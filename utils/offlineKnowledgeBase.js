...
export const getOfflineAnswer = async (query, language = 'english') => {
  ...
  if (isWeb || !db) {
    try {
      ...
      Object.keys(kbData).forEach(key => {
        ...
        if (entry.language === language || (language !== 'english' && entry.language === 'english')) {
          entry.responses.forEach(response => {
            ...
            // Also check if query words match question text
            const questionWords = response.question.toLowerCase().split(/\s+/);
      ...
      if (bestMatch) {
        console.log('✅ Found offline answer with score:', bestScore);
        return bestMatch.answer;
      } else {
        console.log('⚠️ No match found in offline KB, trying fallback...');
...
export const saveChatMessage = async (chatId, userId, message, response, language = 'english', sessionId = null, source = 'offline', confidence = null) => {
  ...
  if (isWeb || !db) {
    try {
      ...
      // Track bot message id separately to avoid reference issues in older runtimes
      ...
      return { userMessageId: userMsg.id, botMessageId };
    } catch (error) {
